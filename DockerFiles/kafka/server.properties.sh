...
KAFKA_IP=`ip addr | grep inet | grep 10.42 | tail -1 | awk '{print $2}' | awk -F\/ '{print $1}'`
echo "################ $KAFKA_IP ####################"
KAFKA_IP_ADDR=${KAFKA_IP_ADDR:-"$KAFKA_IP"}
KAFKA_ADVERTISE_PORT=${KAFKA_ADVERTISE_PORT:-"9092"}
KAFKA_LISTENER=${KAFKA_LISTENER:-"PLAINTEXT://${KAFKA_IP_ADDR}:${KAFKA_ADVERTISE_PORT}"}
